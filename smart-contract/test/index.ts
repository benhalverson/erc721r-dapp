...
async function simulateNextBlockTime(
  baseTime: BigNumber,
  changeBy: BigNumberish
) {
...
const mineSingleBlock = async () => {
  await ethers.provider.send("hardhat_mine", [
    ethers.utils.hexValue(2).toString(),
  ]);
};
...
// }
let blockDeployTimeStamp: number;
...
describe(CollectionConfig.contractName, function () {
  ...
  beforeEach(async function () {
    [owner, whitelistedUser, holder, externalUser, account2, account3] =
      await ethers.getSigners();

    //  merkleTree = getTree(whitelistAddresses);

    blockDeployTimeStamp = (await ethers.provider.getBlock("latest")).timestamp;
  });
  ...
  it("Should be able to mint and request a refund", async function () {
    ...
    const balanceAfterMint = await contract.balanceOf(account2.address);
    expect(balanceAfterMint).to.eq(1);

    const endRefundTime = await contract.getRefundGuaranteeEndTime();

    // console.log("endRefundTime", endRefundTime.valueOf());
    // console.log("block", typeof blockDeployTimeStamp);
    await simulateNextBlockTime(endRefundTime, -20);

    await contract.connect(account2).refund([0]);

    const balanceAfterRefund = await contract.balanceOf(account2.address);
    expect(balanceAfterRefund).to.eq(0);

    const balanceAfterRefundOfOwner = await contract.balanceOf(owner.address);
